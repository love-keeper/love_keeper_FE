...
class ReplyLetterPage extends ConsumerStatefulWidget {
...
class _ReplyLetterPageState extends ConsumerState<ReplyLetterPage> {
  ...
  void initState() {
    ...
    questionTexts = LetterTexts.replyQuestions; // 답장 편지용 질문 사용
  }
      ...
      try {
        ...
        final result = await ref
            ...
            .createDraft(
              draftOrder,
              content,
              draftType: DraftType.answer, // 답장 편지 타입으로 변경
            );
        debugPrint(
                ...
                try {
                  ...
                  if (e is DioException) {
                    if (e.response?.statusCode == 404) {
                      debugPrint(
                        '드래프트 없음 - order: $draftOrder, type: ANSWER (사용자 입력 없음, 무시)',
                      );
                      continue;
                    } else {
                      debugPrint(
                        '드래프트 삭제 실패 - order: $draftOrder, type: ANSWER, Status: ${e.response?.statusCode}, Error: ${e.message}',
                      );
                    }
                  } else {
                    debugPrint(
                      '드래프트 삭제 실패 - order: $draftOrder, type: ANSWER, Error: $e',
                    );
                  }
                }
              }
